...
exports.excuteProcess=function(req,res,next){
    let relayServer = config.relayServerDev;
    let WS=req.wsClient
...
// 带key的数据处理
exports.invokeExternalUrlsDataPcsWithKeys=function(req,res,next){
    ...
    WS.on('message', function incoming(data) {
        ...
        if(data=="node offline"){
          ...
          if(obj['msg']!=undefined&&obj['msg']=='ivkDPcs'){
              if(obj.uid=='none'){
                res.end(JSON.stringify({"code":-1,mssage:'err'}))
              }else if(obj.uid=='none'){
                res.end(JSON.stringify({"code":-1,"url":"none"}))
                 ...
                 try{  
                    let r={}
                    let template
                    for(let b of obj.uid){
                        if(b['dataTemplateId']!=undefined){template=b['dataTemplateId'];continue}
                        r[b.file_name]="http://221.226.60.2:8082/data/"+b.id
                    }
                    res.end(JSON.stringify({"code":0,"urls":r,"dataTemplate":template}))
                 }catch(err){
                    res.end(JSON.stringify({"code":-1,"mssage":err}))
                 }
